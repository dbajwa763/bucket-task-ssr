...
import {BUCKET} from '@constants/ApiConstant';
...
const Buckets = (props) => {
    ...
    const actionHandler = (row,type) => {
        if(type == "edit"){
            ...
            MySwal.fire({title: 'Confirm',html: "Are you sure you want to delete this bucket?",showCancelButton: true,cancelButtonText: "No",confirmButtonText: "Yes"}).then((result) => {
                if(result.isConfirmed){
                    deleteBucket(row.slug);
                }
            });
        }
    }
    ...
    const getData = async(appliedFilters) => {
        try{
            setIsLoading(true);
            const {data} = await axios.get(BUCKET.lists,{params: appliedFilters});
    ...
    const handleSubmit = async () => {
        try{
            ...
            const apiUrl = (form.slug ? BUCKET.update : BUCKET.create);
    ...
    const deleteBucket = async(slug) => {
        try{
            const {data} = await axios.delete(`${BUCKET.delete}/${slug}`);
            if(data.status == c.API_STATUS.SUCCESS){
                toast.success(data.message,toastOptions);
                getData(filters);
            }
    ...
    return (
        <React.Fragment>
            <Head>
                <meta charset="utf-8"/>
                <title>{`Buckets - ${c.APP_NAME}`}</title>
            </Head>
            <Layout page="buckets">
                {showPopup && (
                    <PopupWrap>
                        ...
                        <form className="inner" onSubmit={onSubmit}>
                            <div className="bi-head">
                                <div className="heading">{form.slug ? "Edit" : "Add"} Bucket</div>
                ...
                <BucketWrap>
                    <div className="inner">
                        <div className="head">
                            <h1>Buckets</h1>
                            <button className="btn-wrap" onClick={addBucket}>Add Bucket</button>
                        </div>
...
export const getServerSideProps = async(context) => {
    ...
    try{
        const {data} = await axios.get(BUCKET.lists,{params: queryParams});
        if(data.status == c.API_STATUS.SUCCESS){
